...
import { Mail, Phone, MapPin, Globe, Linkedin, Twitter, Github, ExternalLink, Award, BookOpen, User, Briefcase, GraduationCap, Code } from 'lucide-react';
...
export const AcademicTemplate: React.FC<PreviewProps> = ({ data, language, onChange }) => {
  ...
  return (
    <div id="cv-preview" className="bg-white w-full min-h-[29.7cm] p-8 text-gray-900 font-sans relative">
      ...
      <header className="mb-8">
        <div className="flex flex-col items-center text-center border-b-2 border-blue-800 pb-6">
          {/* Profile Picture */}
          ...
          <div className="text-lg text-blue-800 font-semibold mb-4">
            {renderEditableText(personalInfo.jobTitle || "Academic Title", 'personalInfo', 'jobTitle') || "ACADEMIC TITLE"}
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
        ...
        <div className="lg:col-span-1 space-y-8">
          ...
          {education.length > 0 && (
            <section>
              <div className="flex items-center gap-2 mb-4 pb-2 border-b-2 border-blue-800">
              ...
              <div className="space-y-6">
                {education.map((edu) => (
                  <div key={edu.id} className="border-l-2 border-blue-100 pl-4 py-1">
                    <div
                      className="font-bold text-blue-700 cursor-text border-b border-dashed border-gray-400 hover:border-gray-600"
                      onClick={() => startEditingWithId('education', edu.id, 'degree', edu.degree)}
                    >
                      {renderEditableTextWithId(edu.degree, 'education', edu.id, 'degree')}
                    </div>
                    <div
                      className="text-sm font-semibold text-gray-700 mt-1 cursor-text border-b border-dashed border-gray-400 hover:border-gray-600"
                    ...
                    {edu.description && (
                      <p
                        className="text-xs text-gray-700 mt-1 cursor-text border-b border-dashed border-gray-400 hover:border-gray-600"
            ...
            <section>
              ...
              <div className="space-y-2">
                {skills.map((skill, index) => (
                  <div
                    key={index}
                    className="text-sm px-3 py-1.5 bg-blue-50 text-blue-800 rounded cursor-text"
          ...
          <section>
            <div className="flex items-center gap-2 mb-4 pb-2 border-b-2 border-blue-800">
              ...
              <h2 className="text-xl font-bold font-serif uppercase tracking-wider">
                {language === 'en' ? 'Publications' : 'Xuất bản'}
              </h2>
            </div>
            <div className="text-sm text-gray-700">
              <p>{language === 'en' ? 'Add major publications, research papers, or academic achievements here.' : 'Thêm các bài báo khoa học, nghiên cứu hoặc thành tựu học thuật chính ở đây.'}</p>
            </div>
          </section>
        </div>
        ...
        <div className="lg:col-span-3 space-y-8">
          ...
          {summary && (
            <section>
              ...
              <p
                className="text-justify whitespace-pre-line text-gray-700 cursor-text border-b border-dashed border-gray-400 hover:border-gray-600"
              ...
              <div className="space-y-6">
                {experience.map((exp) => (
                  <div key={exp.id} className="border-l-2 border-blue-100 pl-4 py-1">
                    ...
                    <div
                      className="text-sm font-semibold text-blue-700 cursor-text border-b border-dashed border-gray-400 hover:border-gray-600"
          ...
          {projects.length > 0 && (
            <section>
              ...
              <div className="space-y-6">
                {projects.map((proj) => (
                  <div key={proj.id} className="border-l-2 border-blue-100 pl-4 py-1">
                    <div className="flex justify-between items-baseline">
      ...
      <div className="mt-12 pt-6 border-t border-gray-300 text-center text-sm text-gray-600">
        <p>{language === 'en' ? 'Curriculum Vitae' : 'Sơ yếu lý lịch'} - {new Date().getFullYear()}</p>
      </div>
    </div>
  );
};